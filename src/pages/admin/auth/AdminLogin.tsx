...
export function AdminLogin() {
  ...
  return (
    ...
    >
      ...
      <DemoAccounts onPick={(e, p) => { setEmail(e); setPassword(p); }} highlight="admin" />
      <div className="mt-6 flex items-center justify-center gap-4 text-xs text-cream-3">
        <Link to="/login" className="hover:text-cream">Sou cliente</Link>
        <span aria-hidden>•</span>
        <Link to="/entregador/login" className="hover:text-cream">Sou entregador</Link>
      </div>
    </AuthShell>
  );
}
